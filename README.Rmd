...
d'Alpoim Guedes, Jade and R. Kyle Bocinsky. Research compendium for: *Climate change stimulated agricultural innovation and exchange across Asia*, in review. Version 1.0.0. Zenodo. http://doi.org/10.5281/zenodo.583157

d'Alpoim Guedes, Jade and R. Kyle Bocinsky. Data output for: *Climate change stimulated agricultural innovation and exchange across Asia*, in review. Version 1.0.0. Zenodo. http://doi.org/10.5281/zenodo.583154
...
This repository is a research compendium package for d'Alpoim Guedes and Bocinsky (in review). The compendium contains all code associated with the analyses described and presented in the publication, as well as a Docker environment (described in the `Dockerfile`) for running the code.
...
### Output
The GitHub repository for this project does not contain the output generated by the script---`r "vignettes" %>% list.files(all.files = TRUE, recursive = TRUE, full.names = TRUE) %>% file.size() %>% sum() %>% magrittr::divide_by(1000000000) %>% round(digits = 2)` GB of uncompressed data. All output data is available as a separate Zenodo archive at:
...
Division of Earth and Ecosystem Sciences, Desert Research Institute<br>
The Research Institute at Crow Canyon Archaeological Center<br>
