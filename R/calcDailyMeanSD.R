...
calcDailyMeanSD <- function(daily.data,
  ...
  if (!unwrapped) {
    ...
    daily.data.unwrap <- data.frame(DOY = as.numeric(strftime(as.POSIXlt(paste(rep(daily.data$YEAR, each = 31),
  ...
  mean.predictions <- predict(mean.model,
  ...
  sd.model <- gam(SD ~ s(DOY, bs = "cc"),
                  data = residual.sds)
  sd.predictions <- predict(sd.model,
                            data.frame(DOY = 1:366))

  if (plot) {
    polar.plot(lengths = daily.data.unwrap$DATA,
               ...
               cex = 0.1)
    polar.plot(lengths = as.numeric(mean.predictions),
               ...
               line.col = "dodgerblue",
               add = TRUE)
    polar.plot(lengths = as.numeric(mean.predictions) + as.numeric(sd.predictions),
               polar.pos = as.numeric(names(mean.predictions)) * 360 / 366,
               rp.type = "p",
               clockwise = TRUE,
               start = 0,
